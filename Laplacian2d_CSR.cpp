...
Laplacian2D_CSRMatrix::Laplacian2D_CSRMatrix(const int rows, const int cols) {
...
double Laplacian2D_CSRMatrix::k_func(double x, double y) {
    // Example variable k function; modify as needed
    return 1.0 + 0.5 * (x + y);
}

void Laplacian2D_CSRMatrix::generateMatrix(const int rows, const int cols) 
{
    ...
    CSR rowBlockMatrix_CSR = CSR(rowBlockMatrix);
    ...
    CSR identityMatrixRows_CSR = CSR(identityMatrixRows);

    //     c.) Identity matrix (rows) kronecker product with row block matrix
    Matrix* upperIncidenceMatrix_CSR = identityMatrixRows_CSR.Kronecker(rowBlockMatrix_CSR); // upper incidence matrix
    BlockCSR* rowBlockPtr = static_cast<BlockCSR*>(upperIncidenceMatrix_CSR);  //cast to BlockToeplitz pointer for further operations
    ...
    CSR columnBlockMatrix_CSR = CSR(columnBlockMatrix);
    ...
    CSR identityMatrixCols_CSR = CSR(identityMatrixCols);
    ...
    BlockCSR* lowerIncidencePtr = static_cast<BlockCSR*>(lowerIncidenceMatrix);  //cast to BlockToeplitz pointer for further operations
    ...
    BlockCSR IncidenceMatrix(rows * cols * 2, rows * cols, ndiags); // incidence matrix with 2 block rows
    IncidenceMatrix.Cols[0] = 0; // diagonal for upper incidence matrix
    IncidenceMatrix.Cols[1] = 0; // diagonal for lower incidence matrix
    IncidenceMatrix.Rows[1] = 1;
    IncidenceMatrix.Rows[2] = 2; 
    IncidenceMatrix.Vals[0] = rowBlockPtr;
    IncidenceMatrix.Vals[1] = lowerIncidencePtr;
    //Incidence=&IncidenceMatrix;
    
    //     b.) Compute negative transpose
    Incidence=new BlockCSR(IncidenceMatrix);
...
Vectord Laplacian2D_CSRMatrix::operator*(Vectord& input)
{
    std::cout << "0\n";
...
void Laplacian2D_CSRMatrix::operator*=(double c){throw std::invalid_argument("Not implemented");}
   
Matrix* Laplacian2D_CSRMatrix::Kronecker(Matrix& B) {throw std::invalid_argument("Not implemented");}

Matrix* Laplacian2D_CSRMatrix::Clone(double c) {throw std::invalid_argument("Not implemented");}

Matrix* Laplacian2D_CSRMatrix::negativeTranspose() {throw std::invalid_argument("Not implemented");}
    
void Laplacian2D_CSRMatrix::printFullMatrix() {throw std::invalid_argument("Not implemented");}
    
double Laplacian2D_CSRMatrix::operator()(int i, int j) const {throw std::invalid_argument("Not implemented");}
